...
class mProduct extends mConnection {
    ...
    public function getProduct($ID) {
        $sql = "SELECT PRODUCT.ID, DESCRIPTION, QUANTITY, LOW_STOCK, FK_ID_UNITY, COST, SALE_VALUE, ON_DEMAND, FK_ID_PROVIDER, OBSERVATION, PRODUCT.CREATED_BY, PRODUCT.CREATED_ON, PRODUCT.MODIFIED_BY, PRODUCT.MODIFIED_ON, UNITY.NAME, ABREVIATION  FROM PRODUCT INNER JOIN UNITY ON UNITY.ID = PRODUCT.FK_ID_UNITY WHERE PRODUCT.ID = :ID";
        $con = $this->Connect();
        $stmt = $con->prepare($sql);
        $stmt->bindParam(":ID", $ID, PDO::PARAM_INT);
        $stmt->execute();
