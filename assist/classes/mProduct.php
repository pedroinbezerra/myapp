...
class mProduct extends mConnection {
    ...
    public function createProduct($DESCRIPTION, $QUANTITY, $LOW_STOCK, $FK_ID_UNITY, $MEASURE, $COST, $SALE_VALUE, $ON_DEMAND, $FK_ID_PROVIDER, $OBSERVATION, $CREATED_BY) {
        $sql = "INSERT INTO PRODUCT(DESCRIPTION, QUANTITY, LOW_STOCK, FK_ID_UNITY, MEASURE, COST, SALE_VALUE, ON_DEMAND, FK_ID_PROVIDER, OBSERVATION, CREATED_BY) VALUES(:DESCRIPTION, :QUANTITY, :LOW_STOCK, :FK_ID_UNITY, :MEASURE, :COST, :SALE_VALUE, :ON_DEMAND, :FK_ID_PROVIDER, :OBSERVATION, :CREATED_BY)";
        $con = $this->Connect();
    ...
    public function getProduct($ID) {
        $sql = "SELECT PRODUCT.ID, DESCRIPTION, QUANTITY, LOW_STOCK, FK_ID_UNITY, COST, SALE_VALUE, ON_DEMAND, FK_ID_PROVIDER, OBSERVATION, PRODUCT.CREATED_BY, PRODUCT.CREATED_ON, PRODUCT.MODIFIED_BY, PRODUCT.MODIFIED_ON, UNITY.NAME, ABREVIATION, MEASURE  FROM PRODUCT INNER JOIN UNITY ON UNITY.ID = PRODUCT.FK_ID_UNITY WHERE PRODUCT.ID = :ID";
        $con = $this->Connect();
        $stmt = $con->prepare($sql);
        $stmt->bindParam(":ID", $ID, PDO::PARAM_INT);
        $stmt->execute();
    ...
    public function getProviderName($ID) {
        ...
        $stmt->bindParam(":ID", $ID, PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_ASSOC)['NAME'] ?? 'Fornecedor não encontrado';
    }
